...
It opens in Russian. A switch in the top bar changes it to Kazakh or English and remembers the
choice. The evidence strings, hypotheses and reasons are generated by the pipeline in English and
are shown as they are. Two colour schemes, paper by default and dark for a projector, both drawn
...
| 02 | Who to review first | The 25 ranked accounts, each with its role and the rule's own numbers |
| 03 | One account | Who paid it on the left, who it paid on the right, arrows the way the money moved, width by KZT. The panel beside it reads as sentences |
| 04 | Structures | The 653 formations ranked, each on its own map with the breaking point in amber |
| 05 | Replay the month | July day by day on the hop layout: play, scrub, a 31-bar strip of daily KZT, a fading trail of the previous days |
| 06 | Amount echoes | The 203 echoes as in → account → out diagrams with the amounts on the lines |
| 07 | Where the data runs out | The boundary figures, the ranked data requests, where centrality misleads, the removal test |

Hovering any role name shows its definition in one sentence.
